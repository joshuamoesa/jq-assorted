...
get_aws_creds () {
    tmp_role=$(echo cc-flow-$1-acc | cut -c1-60)
    export temp_credentials=$(aws sts assume-role --role-arn $AWSTESTENVROLE --role-session-name $tmp_role)
